...
public class OgreTitanRenderer extends GeoEntityRenderer<OgreTitanEntity> {
   private static final Map<Integer, Long> lastSendTick = new HashMap<>();

   public OgreTitanRenderer(Context context) {
      super(context, new OgreTitanModel());
      this.shadowRadius = 3.0F;
   }

   public void render(OgreTitanEntity entity, float entityYaw, float partialTick, MatrixStack poseStack, VertexConsumerProvider bufferSource, int packedLight) {
      super.render(entity, entityYaw, partialTick, poseStack, bufferSource, packedLight);
      this.syncHitboxBones(entity, partialTick);
   }

   private void syncHitboxBones(OgreTitanEntity entity, float partialTick) {
         ...
         if (lastTick == null || lastTick != currentTick) {
            ...
            if (!(distSq > 4096.0)) {
               ...
               if (napeBone != null) {
                  ...
                  OgreTitanNapeEntity napeEntity = OgreTitanNapeEntity.getClientInstance(entity.getId());
                  if (napeEntity != null) {
   ...
   protected float getDeathMaxRotation(OgreTitanEntity animatable) {
      return 0.0F;
   }

   public int getPackedOverlay(OgreTitanEntity animatable, float u, float partialTick) {
