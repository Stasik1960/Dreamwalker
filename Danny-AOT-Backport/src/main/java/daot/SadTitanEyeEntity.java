...
public class SadTitanEyeEntity extends MobEntity implements PureTitanHitbox {
   private static final TrackedData<Optional<UUID>> DATA_PARENT_UUID = DataTracker.registerData(
      SadTitanEyeEntity.class, TrackedDataHandlerRegistry.OPTIONAL_UUID
   );
   private SadTitanEntity cachedParent;
   private static final double EYE_OFFSET_FORWARD = 0.65;
   private static final double EYE_HEIGHT_OFFSET = 3.2;
   private static final Map<Integer, SadTitanEyeEntity> clientInstances = new ConcurrentHashMap<>();
   ...
   public static SadTitanEyeEntity getClientInstance(int parentEntityId) {
   ...
   private void positionFromStaticOffset(SadTitanEntity parent, double scale) {
   ...
   public SadTitanEyeEntity(EntityType<? extends MobEntity> entityType, World level) {
   ...
   public void setParentTitan(SadTitanEntity titan) {
   ...
   public SadTitanEntity getParentTitan() {
      ...
      if (parentUUID.isEmpty()) {
         ...
         if (this.getWorld() instanceof ServerWorld serverLevel && serverLevel.getEntity(uuid) instanceof SadTitanEntity titan && titan.isAlive()) {
            this.cachedParent = titan;
            return titan;
         } else {
            if (this.getWorld().isClient()) {
               for (Entity entity : this.getWorld().getOtherEntities(this, this.getBoundingBox().expand(20.0))) {
                  if (entity instanceof SadTitanEntity titan && titan.getUuid().equals(uuid)) {
                     this.cachedParent = titan;
                     return titan;
                  }
               }
   ...
   public void tick() {
      super.tick();
      SadTitanEntity parent = this.getParentTitan();
      if (parent == null) {
   ...
   public boolean damage(DamageSource source, float amount) {
      if (this.getWorld().isClient()) {
         ...
         SadTitanEntity stunCheckParent = this.getParentTitan();
         if (stunCheckParent != null && stunCheckParent.isEating() && !ModConfig.get().canStunWhileGrabbed) {
            ...
            if (!(attacker instanceof AttackTitanEntity)
               ...
               SadTitanEntity ownerCheckParent = this.getParentTitan();
               ...
               if (source.isIn(DamageTypeTags.IS_EXPLOSION)) {
                  ...
                  if (attacker instanceof LivingEntity livingAttacker) {
                     ...
                     if (parent != null && !parent.isDead() && !parent.isEyeHurt()) {
                        ...
                        parent.triggerEyeHurt();
                        if (livingAttacker instanceof PlayerEntity player) {
                           player.sendMessage(Text.literal("Critical hit! Sad Titan's eye damaged - stunned for 5 seconds!"), true);
                        }
   ...
   public boolean collidesWith(Entity other) {
      return other instanceof SadTitanEntity titan ? titan != this.getParentTitan() : super.collidesWith(other);
   }
