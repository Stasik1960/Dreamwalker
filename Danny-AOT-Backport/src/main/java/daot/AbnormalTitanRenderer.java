...
public class AbnormalTitanRenderer extends GeoEntityRenderer<AbnormalTitanEntity> {
   ...
   public AbnormalTitanRenderer(Context context) {
      super(context, new AbnormalTitanModel());
   }

   public void render(
      AbnormalTitanEntity entity, float entityYaw, float partialTick, MatrixStack poseStack, VertexConsumerProvider bufferSource, int packedLight
   ) {
      super.render(entity, entityYaw, partialTick, poseStack, bufferSource, packedLight);
      this.syncHitboxBones(entity, partialTick);
   }

   private void syncHitboxBones(AbnormalTitanEntity entity, float partialTick) {
            ...
            if (!(distSq > 4096.0)) {
               ...
               if (napeBone == null) {
                  if (!loggedMissing.containsKey(entity.getId())) {
                     ...
                     System.out.println("[AbnormalTitanHitboxSync] WARNING: nape_hitbox bone not found in model!");
                  }
               } else {
   ...
   protected float getDeathMaxRotation(AbnormalTitanEntity animatable) {
      return 0.0F;
   }

   public int getPackedOverlay(AbnormalTitanEntity animatable, float u, float partialTick) {
