...
public class BeastTitanGrabEntity extends MobEntity {
   private static final TrackedData<Optional<UUID>> DATA_PARENT_UUID = DataTracker.registerData(
      BeastTitanGrabEntity.class, TrackedDataHandlerRegistry.OPTIONAL_UUID
   );
   private BeastTitanEntity cachedParent;
   private boolean allowDismount = false;
   private static final double GRAB_HEIGHT_OFFSET = 10.0;
   private static final Map<Integer, BeastTitanGrabEntity> clientInstances = new ConcurrentHashMap<>();
   ...
   public static BeastTitanGrabEntity getClientInstance(int parentEntityId) {
   ...
   private void positionFromStaticOffset(BeastTitanEntity parent) {
      ...
      double fwdX = -Math.sin(yawRad) * 5.0;
      double fwdZ = Math.cos(yawRad) * 5.0;
      this.setPosition(parent.getX() + fwdX, parent.getY() + 10.0, parent.getZ() + fwdZ);
   }

   public BeastTitanGrabEntity(EntityType<? extends MobEntity> entityType, World level) {
