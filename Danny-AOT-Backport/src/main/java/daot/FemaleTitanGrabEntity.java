...
public class FemaleTitanGrabEntity extends MobEntity {
   private static final TrackedData<Optional<UUID>> DATA_PARENT_UUID = DataTracker.registerData(
      FemaleTitanGrabEntity.class, TrackedDataHandlerRegistry.OPTIONAL_UUID
   );
   private FemaleTitanEntity cachedParent;
   private boolean allowDismount = false;
   private static final double GRAB_HEIGHT_OFFSET = 8.0;
   private static final Map<Integer, FemaleTitanGrabEntity> clientInstances = new ConcurrentHashMap<>();
   ...
   public static FemaleTitanGrabEntity getClientInstance(int parentEntityId) {
   ...
   private void positionFromStaticOffset(FemaleTitanEntity parent) {
      ...
      this.setPosition(parent.getX() + fwdX, parent.getY() + 8.0, parent.getZ() + fwdZ);
   }

   public FemaleTitanGrabEntity(EntityType<? extends MobEntity> entityType, World level) {
