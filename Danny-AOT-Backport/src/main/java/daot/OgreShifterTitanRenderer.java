...
public class OgreShifterTitanRenderer extends GeoEntityRenderer<OgreShifterTitanEntity> {
   private static final Map<Integer, Long> lastSendTick = new HashMap<>();

   public OgreShifterTitanRenderer(Context context) {
      super(context, new OgreShifterTitanModel());
      this.shadowRadius = 3.0F;
   }

   public void render(
      OgreShifterTitanEntity entity, float entityYaw, float partialTick, MatrixStack poseStack, VertexConsumerProvider bufferSource, int packedLight
   ) {
      super.render(entity, entityYaw, partialTick, poseStack, bufferSource, packedLight);
      this.syncHitboxBones(entity, partialTick);
   }

   private void syncHitboxBones(OgreShifterTitanEntity entity, float partialTick) {
   ...
   protected float getDeathMaxRotation(OgreShifterTitanEntity animatable) {
      return 0.0F;
   }

   public int getPackedOverlay(OgreShifterTitanEntity animatable, float u, float partialTick) {
