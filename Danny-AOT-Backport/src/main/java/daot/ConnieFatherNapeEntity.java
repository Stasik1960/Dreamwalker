...
public class ConnieFatherNapeEntity extends MobEntity {
   private static final TrackedData<Optional<UUID>> DATA_PARENT_UUID = DataTracker.registerData(
      ConnieFatherNapeEntity.class, TrackedDataHandlerRegistry.OPTIONAL_UUID
   );
   private ConnieFatherEntity cachedParent;
   private static final double NAPE_OFFSET_BACK = 0.4;
   private static final double NAPE_HEIGHT_OFFSET = 1.8;
   private static final Map<Integer, ConnieFatherNapeEntity> clientInstances = new ConcurrentHashMap<>();
   ...
   public static ConnieFatherNapeEntity getClientInstance(int parentEntityId) {
   ...
   private void positionFromStaticOffset(ConnieFatherEntity parent, double scale) {
      ...
      double scaledBackOffset = 0.4 * scale;
      double offsetX = Math.sin(yawRad) * scaledBackOffset;
      double offsetZ = -Math.cos(yawRad) * scaledBackOffset;
      this.setPosition(parent.getX() + offsetX, parent.getY() + 1.8 * scale, parent.getZ() + offsetZ);
   }

   public ConnieFatherNapeEntity(EntityType<? extends MobEntity> entityType, World level) {
   ...
   public boolean damage(DamageSource source, float amount) {
      if (this.getWorld().isClient()) {
         ...
         if (!(attacker instanceof AttackTitanEntity)
            ...
            ConnieFatherEntity unfairParent = this.getParentTitan();
               ...
               if (parent != null && parent.hasPassenger(attacker)) {
                  ...
                  if ((isUsingBlade || isHomelander) && isMeleeRange && parent != null) {
                     ...
                     if (livingAttacker instanceof PlayerEntity player) {
                        player.sendMessage(Text.literal("Critical hit! Connie's Father Titan's nape destroyed!"), true);
                     }
