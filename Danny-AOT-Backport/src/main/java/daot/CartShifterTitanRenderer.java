...
public class CartShifterTitanRenderer extends GeoEntityRenderer<CartShifterTitanEntity> {
   ...
   public CartShifterTitanRenderer(Context context) {
      super(context, new CartShifterTitanModel());
      this.shadowRadius = 1.5F;
   }

   public void render(
      CartShifterTitanEntity entity, float entityYaw, float partialTick, MatrixStack poseStack, VertexConsumerProvider bufferSource, int packedLight
   ) {
   ...
   private void snapGrabbedToBone(CartShifterTitanEntity entity, float partialTick) {
      if (entity.isGrabbing()) {
         ...
         if (grab != null && !grab.getPassengerList().isEmpty()) {
            GeoBone grabBone = ((software.bernie.geckolib.cache.object.GeoBone) this.getGeoModel().getAnimationProcessor().getBone("mouth_grab"));
            if (grabBone != null) {
   ...
   private static boolean isLocalRiderFirstPerson(CartShifterTitanEntity entity) {
   ...
   private void captureHeadBoneWorldPos(CartShifterTitanEntity entity, float partialTick) {
   ...
   private void syncHitboxBones(CartShifterTitanEntity entity, float partialTick) {
      MinecraftClient mc = MinecraftClient.getInstance();
   ...
   protected float getDeathMaxRotation(CartShifterTitanEntity animatable) {
      return 0.0F;
   }

   public int getPackedOverlay(CartShifterTitanEntity animatable, float u, float partialTick) {
