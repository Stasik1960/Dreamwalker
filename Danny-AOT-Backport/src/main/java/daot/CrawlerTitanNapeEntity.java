...
public class CrawlerTitanNapeEntity extends MobEntity {
   private static final TrackedData<Optional<UUID>> DATA_PARENT_UUID = DataTracker.registerData(
      CrawlerTitanNapeEntity.class, TrackedDataHandlerRegistry.OPTIONAL_UUID
   );
   private CrawlerTitanEntity cachedParent;
   private static final double NAPE_OFFSET_BACK = 0.8;
   private static final double NAPE_HEIGHT_OFFSET = 1.5;
   private static final Map<Integer, CrawlerTitanNapeEntity> clientInstances = new ConcurrentHashMap<>();
   ...
   public static CrawlerTitanNapeEntity getClientInstance(int parentEntityId) {
   ...
   private void positionFromStaticOffset(CrawlerTitanEntity parent, double scale) {
      ...
      double scaledBackOffset = 0.8 * scale;
      double offsetX = Math.sin(yawRad) * scaledBackOffset;
      double offsetZ = -Math.cos(yawRad) * scaledBackOffset;
      this.setPosition(parent.getX() + offsetX, parent.getY() + 1.5 * scale, parent.getZ() + offsetZ);
   }

   public CrawlerTitanNapeEntity(EntityType<? extends MobEntity> entityType, World level) {
   ...
   public boolean damage(DamageSource source, float amount) {
      if (this.getWorld().isClient()) {
         ...
         if (!(attacker instanceof AttackTitanEntity)
            ...
            CrawlerTitanEntity unfairParent = this.getParentTitan();
               ...
               if (parent != null && parent.hasPassenger(attacker)) {
                  ...
                  if ((isUsingBlade || isHomelander) && isMeleeRange && parent != null) {
                     ...
                     if (livingAttacker instanceof PlayerEntity player) {
                        player.sendMessage(Text.literal("Critical hit! Crawler Titan's nape destroyed!"), true);
                     }
