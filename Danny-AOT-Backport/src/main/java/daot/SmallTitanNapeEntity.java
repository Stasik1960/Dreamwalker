...
public class SmallTitanNapeEntity extends MobEntity {
   private static final TrackedData<Optional<UUID>> DATA_PARENT_UUID = DataTracker.registerData(
      SmallTitanNapeEntity.class, TrackedDataHandlerRegistry.OPTIONAL_UUID
   );
   private SmallTitanEntity cachedParent;
   private static final double NAPE_OFFSET_BACK = 0.5;
   private static final double NAPE_HEIGHT_OFFSET = 2.0;
   private static final Map<Integer, SmallTitanNapeEntity> clientInstances = new ConcurrentHashMap<>();
   ...
   public static SmallTitanNapeEntity getClientInstance(int parentEntityId) {
   ...
   private void positionFromStaticOffset(SmallTitanEntity parent, double scale) {
      ...
      double scaledBackOffset = 0.5 * scale;
   ...
   public SmallTitanNapeEntity(EntityType<? extends MobEntity> entityType, World level) {
   ...
   public boolean damage(DamageSource source, float amount) {
      if (this.getWorld().isClient()) {
         ...
         if (!(attacker instanceof AttackTitanEntity)
            ...
            SmallTitanEntity unfairParent = this.getParentTitan();
               ...
               if (parent != null && parent.hasPassenger(attacker)) {
                  ...
                  if ((isUsingBlade || isHomelander) && isMeleeRange && parent != null) {
                     ...
                     if (livingAttacker instanceof PlayerEntity player) {
                        player.sendMessage(Text.literal("Critical hit! Small Titan's nape destroyed!"), true);
                     }
