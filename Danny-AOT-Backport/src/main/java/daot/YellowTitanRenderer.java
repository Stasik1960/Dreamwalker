...
public class YellowTitanRenderer extends GeoEntityRenderer<YellowTitanEntity> {
   private static final Map<Integer, Long> lastSendTick = new HashMap<>();

   public YellowTitanRenderer(Context context) {
      super(context, new YellowTitanModel());
   }

   public void render(YellowTitanEntity entity, float entityYaw, float partialTick, MatrixStack poseStack, VertexConsumerProvider bufferSource, int packedLight) {
      super.render(entity, entityYaw, partialTick, poseStack, bufferSource, packedLight);
      this.syncHitboxBones(entity, partialTick);
   }

   private void syncHitboxBones(YellowTitanEntity entity, float partialTick) {
         ...
         if (lastTick == null || lastTick != currentTick) {
            ...
            if (!(distSq > 4096.0)) {
               ...
               if (napeBone != null || eyeBone != null) {
                  ...
                  YellowTitanNapeEntity napeEntity = YellowTitanNapeEntity.getClientInstance(entity.getId());
                  if (napeEntity != null && napeBone != null) {
                     napeEntity.setRendererPosition(napeX, napeY, napeZ);
                  }

                  YellowTitanEyeEntity eyeEntity = YellowTitanEyeEntity.getClientInstance(entity.getId());
   ...
   protected float getDeathMaxRotation(YellowTitanEntity animatable) {
      return 0.0F;
   }

   public int getPackedOverlay(YellowTitanEntity animatable, float u, float partialTick) {
