...
public class ColossalTitanEyeEntity extends MobEntity {
   private static final TrackedData<Optional<UUID>> DATA_PARENT_UUID = DataTracker.registerData(
      ColossalTitanEyeEntity.class, TrackedDataHandlerRegistry.OPTIONAL_UUID
   );
   private ColossalTitanEntity cachedParent;
   private static final double EYE_OFFSET_FORWARD = 8.0;
   private static final double EYE_HEIGHT_OFFSET = 56.0;
   private static final Map<Integer, ColossalTitanEyeEntity> clientInstances = new ConcurrentHashMap<>();
   ...
   public static ColossalTitanEyeEntity getClientInstance(int parentEntityId) {
   ...
   private void positionFromStaticOffset(ColossalTitanEntity parent) {
      ...
      double offsetX = -Math.sin(yawRad) * 8.0;
      double offsetZ = Math.cos(yawRad) * 8.0;
      this.setPosition(parent.getX() + offsetX, parent.getY() + 56.0, parent.getZ() + offsetZ);
   }

   public ColossalTitanEyeEntity(EntityType<? extends MobEntity> entityType, World level) {
      ...
      if (this.getWorld().isClient()) {
         ...
         if (attacker instanceof LivingEntity livingAttacker) {
            ...
            if (parent != null && !parent.isBlinded()) {
               ...
               parent.triggerBlindness();
               if (livingAttacker instanceof PlayerEntity player) {
                  player.sendMessage(Text.literal("Critical hit! Colossal Titan blinded for 6 seconds!"), true);
               }
