...
public class FemaleTitanRenderer extends GeoEntityRenderer<FemaleTitanEntity> {
   ...
   public FemaleTitanRenderer(Context renderManager) {
      super(renderManager, new FemaleTitanModel());
      this.shadowRadius = 2.0F;
   }

   public void render(FemaleTitanEntity entity, float entityYaw, float partialTick, MatrixStack poseStack, VertexConsumerProvider bufferSource, int packedLight) {
   ...
   public boolean shouldRender(FemaleTitanEntity entity, Frustum frustum, double camX, double camY, double camZ) {
      MinecraftClient mc = MinecraftClient.getInstance();
      return mc.player != null && mc.player.getVehicle() == entity ? true : super.shouldRender(entity, frustum, camX, camY, camZ);
   }

   private void snapGrabbedToBone(FemaleTitanEntity entity, float partialTick) {
      if (entity.getGrabPhase() != 0) {
         FemaleTitanGrabEntity grab = FemaleTitanGrabEntity.getClientInstance(entity.getId());
         if (grab != null) {
   ...
   private static float hardeningProgress(FemaleTitanEntity entity, float partialTick) {
   ...
   private static boolean isTransitioning(FemaleTitanEntity entity, float partialTick) {
   ...
   public void actuallyRender(
      ...
      if (!isReRender && isTransitioning(animatable, partialTick)) {
         ...
         int newAlpha = clampAlpha((int)(progress * 255.0F));
         Identifier oldTex = FemaleTitanModel.textureFor(animatable.getPreviousHardeningMode());
         ...
         Identifier newTex = FemaleTitanModel.textureFor(animatable.getDisplayedHardeningMode());
   ...
   private void syncHitboxBones(FemaleTitanEntity entity, float partialTick) {
      MinecraftClient mc = MinecraftClient.getInstance();
         ...
         if (lastTick == null || lastTick != currentTick) {
            ...
            if (!(distSq > 4096.0)) {
               ...
               if (napeBone != null || eyeBone != null) {
                  ...
                  FemaleTitanNapeEntity napeEntity = FemaleTitanNapeEntity.getClientInstance(entity.getId());
                  if (napeEntity != null && napeBone != null) {
                     napeEntity.setRendererPosition(napeX, napeY, napeZ);
                  }

                  FemaleTitanEyeEntity eyeEntity = FemaleTitanEyeEntity.getClientInstance(entity.getId());
                  ...
                  if (grabBone != null) {
                     ...
                     FemaleTitanGrabEntity grabEntity = FemaleTitanGrabEntity.getClientInstance(entity.getId());
                     if (grabEntity != null) {
