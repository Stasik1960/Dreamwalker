...
public class FritzTitanEyeEntity extends MobEntity implements PureTitanHitbox {
   private static final TrackedData<Optional<UUID>> DATA_PARENT_UUID = DataTracker.registerData(
      FritzTitanEyeEntity.class, TrackedDataHandlerRegistry.OPTIONAL_UUID
   );
   private FritzTitanEntity cachedParent;
   private static final double EYE_OFFSET_FORWARD = 1.5;
   private static final double EYE_HEIGHT_OFFSET = 13.0;
   ...
   private static final Map<Integer, FritzTitanEyeEntity> clientInstances = new ConcurrentHashMap<>();
   private boolean rendererPositionedThisTick = false;

   public static FritzTitanEyeEntity getClientInstance(int parentEntityId) {
   ...
   public FritzTitanEyeEntity(EntityType<? extends MobEntity> entityType, World level) {
   ...
   private void positionFromStaticOffset(FritzTitanEntity parent, double scale) {
      ...
      double scaledForwardOffset = 1.5 * scale;
      double offsetX = -Math.sin(yawRad) * scaledForwardOffset;
      double offsetZ = Math.cos(yawRad) * scaledForwardOffset;
      this.setPosition(parent.getX() + offsetX, parent.getY() + 13.0 * scale, parent.getZ() + offsetZ);
   }
   ...
   public boolean damage(DamageSource source, float amount) {
      if (this.getWorld().isClient()) {
         ...
         FritzTitanEntity stunCheckParent = this.getParentTitan();
         if (stunCheckParent != null && stunCheckParent.isEating() && !ModConfig.get().canStunWhileGrabbed) {
            ...
            if (!(attacker instanceof AttackTitanEntity)
               ...
               FritzTitanEntity ownerCheckParent = this.getParentTitan();
                  ...
                  if (attacker instanceof LivingEntity livingAttacker) {
                     ...
                     if (parent != null && !parent.isDead() && !parent.isEyeHurt()) {
                        ...
                        if (livingAttacker instanceof PlayerEntity player) {
                           ...
                           player.sendMessage(Text.literal("Critical hit! " + titanName + "'s eye damaged - stunned for 5 seconds!"), true);
                        }
