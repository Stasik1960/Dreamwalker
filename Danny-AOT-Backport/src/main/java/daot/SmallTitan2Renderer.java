...
public class SmallTitan2Renderer extends GeoEntityRenderer<SmallTitan2Entity> {
   private static final Map<Integer, Long> lastSendTick = new HashMap<>();

   public SmallTitan2Renderer(Context context) {
      super(context, new SmallTitan2Model());
   }

   public void render(SmallTitan2Entity entity, float entityYaw, float partialTick, MatrixStack poseStack, VertexConsumerProvider bufferSource, int packedLight) {
      super.render(entity, entityYaw, partialTick, poseStack, bufferSource, packedLight);
      this.syncHitboxBones(entity, partialTick);
   }

   private void syncHitboxBones(SmallTitan2Entity entity, float partialTick) {
         ...
         if (lastTick == null || lastTick != currentTick) {
            ...
            if (!(distSq > 4096.0)) {
               ...
               if (napeBone != null || eyeBone != null) {
                  ...
                  SmallTitan2NapeEntity napeEntity = SmallTitan2NapeEntity.getClientInstance(entity.getId());
                  if (napeEntity != null && napeBone != null) {
                     napeEntity.setRendererPosition(napeX, napeY, napeZ);
                  }

                  SmallTitan2EyeEntity eyeEntity = SmallTitan2EyeEntity.getClientInstance(entity.getId());
   ...
   protected float getDeathMaxRotation(SmallTitan2Entity animatable) {
      return 0.0F;
   }

   public int getPackedOverlay(SmallTitan2Entity animatable, float u, float partialTick) {
