...
public class TripleTTitanRenderer extends GeoEntityRenderer<TripleTTitanEntity> {
   private static final Map<Integer, Long> lastSendTick = new HashMap<>();

   public TripleTTitanRenderer(Context renderManager) {
      super(renderManager, new TripleTTitanModel());
      this.shadowRadius = 2.0F;
   }

   public void render(
      TripleTTitanEntity entity, float entityYaw, float partialTick, MatrixStack poseStack, VertexConsumerProvider bufferSource, int packedLight
   ) {
      super.render(entity, entityYaw, partialTick, poseStack, bufferSource, packedLight);
      this.syncHitboxBones(entity, partialTick);
   }

   private void syncHitboxBones(TripleTTitanEntity entity, float partialTick) {
