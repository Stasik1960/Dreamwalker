...
public class SmallTitanRenderer extends GeoEntityRenderer<SmallTitanEntity> {
   private static final Map<Integer, Long> lastSendTick = new HashMap<>();

   public SmallTitanRenderer(Context context) {
      super(context, new SmallTitanModel());
   }

   public void render(SmallTitanEntity entity, float entityYaw, float partialTick, MatrixStack poseStack, VertexConsumerProvider bufferSource, int packedLight) {
      super.render(entity, entityYaw, partialTick, poseStack, bufferSource, packedLight);
      this.syncHitboxBones(entity, partialTick);
   }

   private void syncHitboxBones(SmallTitanEntity entity, float partialTick) {
         ...
         if (lastTick == null || lastTick != currentTick) {
            ...
            if (!(distSq > 4096.0)) {
               ...
               if (napeBone != null || eyeBone != null) {
                  ...
                  SmallTitanNapeEntity napeEntity = SmallTitanNapeEntity.getClientInstance(entity.getId());
                  if (napeEntity != null && napeBone != null) {
                     napeEntity.setRendererPosition(napeX, napeY, napeZ);
                  }

                  SmallTitanEyeEntity eyeEntity = SmallTitanEyeEntity.getClientInstance(entity.getId());
   ...
   protected float getDeathMaxRotation(SmallTitanEntity animatable) {
      return 0.0F;
   }

   public int getPackedOverlay(SmallTitanEntity animatable, float u, float partialTick) {
