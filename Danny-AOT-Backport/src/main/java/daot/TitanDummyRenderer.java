...
public class TitanDummyRenderer extends GeoEntityRenderer<TitanDummyEntity> {
   private static final Map<Integer, Long> lastSendTick = new HashMap<>();

   public TitanDummyRenderer(Context context) {
      super(context, new TitanDummyModel());
   }

   public void render(TitanDummyEntity entity, float entityYaw, float partialTick, MatrixStack poseStack, VertexConsumerProvider bufferSource, int packedLight) {
      super.render(entity, entityYaw, partialTick, poseStack, bufferSource, packedLight);
      this.syncHitboxBones(entity, partialTick);
   }

   private void syncHitboxBones(TitanDummyEntity entity, float partialTick) {
         ...
         if (lastTick == null || lastTick != currentTick) {
            ...
            if (!(distSq > 4096.0)) {
               ...
               if (napeBone != null || eyeBone != null) {
                  ...
                  double napeX = entityPos.x;
                  double napeY = entityPos.y;
                  double napeZ = entityPos.z;
                  double eyeX = entityPos.x;
                  double eyeY = entityPos.y;
                  double eyeZ = entityPos.z;
                  if (napeBone != null) {
                  ...
                  TitanDummyNapeEntity napeEntity = TitanDummyNapeEntity.getClientInstance(entity.getId());
                  if (napeEntity != null && napeBone != null) {
                     napeEntity.setRendererPosition(napeX, napeY, napeZ);
                  }

                  TitanDummyEyeEntity eyeEntity = TitanDummyEyeEntity.getClientInstance(entity.getId());
