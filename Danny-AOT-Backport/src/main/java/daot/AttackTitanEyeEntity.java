...
public class AttackTitanEyeEntity extends MobEntity {
   private static final TrackedData<Optional<UUID>> DATA_PARENT_UUID = DataTracker.registerData(
      AttackTitanEyeEntity.class, TrackedDataHandlerRegistry.OPTIONAL_UUID
   );
   ...
   private static final Map<Integer, AttackTitanEyeEntity> clientInstances = new ConcurrentHashMap<>();
   ...
   public static AttackTitanEyeEntity getClientInstance(int parentEntityId) {
   ...
   private void positionFromStaticOffset(AttackTitanEntity parent) {
      ...
      this.setPosition(parent.getX() + offsetX, parent.getY() + 13.5, parent.getZ() + offsetZ);
   }

   public AttackTitanEyeEntity(EntityType<? extends MobEntity> entityType, World level) {
      ...
      if (this.getWorld().isClient()) {
         ...
         if (attacker instanceof LivingEntity livingAttacker) {
            ...
            if (parent != null && !parent.isBlinded()) {
               ...
               if (livingAttacker instanceof PlayerEntity player) {
                  player.sendMessage(Text.literal("Critical hit! Attack Titan blinded for 6 seconds!"), true);
               }
