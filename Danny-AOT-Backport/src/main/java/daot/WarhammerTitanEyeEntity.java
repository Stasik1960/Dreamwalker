...
public class WarhammerTitanEyeEntity extends MobEntity {
   private static final TrackedData<Optional<UUID>> DATA_PARENT_UUID = DataTracker.registerData(
      WarhammerTitanEyeEntity.class, TrackedDataHandlerRegistry.OPTIONAL_UUID
   );
   private WarhammerTitanEntity cachedParent;
   private static final double EYE_OFFSET_FORWARD = 2.0;
   private static final double EYE_HEIGHT_OFFSET = 11.5;
   private static final Map<Integer, WarhammerTitanEyeEntity> clientInstances = new ConcurrentHashMap<>();
   ...
   public static WarhammerTitanEyeEntity getClientInstance(int parentEntityId) {
   ...
   private void positionFromStaticOffset(WarhammerTitanEntity parent) {
      float yawRad = (float)Math.toRadians(parent.bodyYaw);
      double offsetX = -Math.sin(yawRad) * 2.0;
      double offsetZ = Math.cos(yawRad) * 2.0;
      this.setPosition(parent.getX() + offsetX, parent.getY() + 11.5, parent.getZ() + offsetZ);
   }

   public WarhammerTitanEyeEntity(EntityType<? extends MobEntity> entityType, World level) {
      ...
      if (this.getWorld().isClient()) {
         ...
         if (attacker instanceof LivingEntity livingAttacker) {
            ...
            if (parent != null && !parent.isBlinded()) {
               ...
               if (livingAttacker instanceof PlayerEntity player) {
                  player.sendMessage(Text.literal("Critical hit! Warhammer Titan blinded for 6 seconds!"), true);
               }
