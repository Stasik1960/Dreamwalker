...
public class SadTitanNapeEntity extends MobEntity {
   private static final TrackedData<Optional<UUID>> DATA_PARENT_UUID = DataTracker.registerData(
      SadTitanNapeEntity.class, TrackedDataHandlerRegistry.OPTIONAL_UUID
   );
   ...
   private static final Map<Integer, SadTitanNapeEntity> clientInstances = new ConcurrentHashMap<>();
   ...
   public static SadTitanNapeEntity getClientInstance(int parentEntityId) {
   ...
   public SadTitanNapeEntity(EntityType<? extends MobEntity> entityType, World level) {
   ...
   public boolean damage(DamageSource source, float amount) {
      if (this.getWorld().isClient()) {
         ...
         if (!(attacker instanceof AttackTitanEntity)
            ...
            SadTitanEntity unfairParent = this.getParentTitan();
               ...
               if (parent != null && parent.hasPassenger(attacker)) {
                  ...
                  if ((isUsingBlade || isHomelander) && isMeleeRange && parent != null) {
                     ...
                     if (livingAttacker instanceof PlayerEntity player) {
                        player.sendMessage(Text.literal("Critical hit! Sad Titan's nape destroyed!"), true);
                     }
