...
public class TitanEyeEntity extends MobEntity implements PureTitanHitbox {
   private static final TrackedData<Optional<UUID>> DATA_PARENT_UUID = DataTracker.registerData(TitanEyeEntity.class, TrackedDataHandlerRegistry.OPTIONAL_UUID);
   private TitanEntity cachedParent;
   private static final double EYE_OFFSET_FORWARD = 2.0;
   private static final double EYE_HEIGHT_OFFSET = 13.5;
   private static final Map<Integer, TitanEyeEntity> clientInstances = new ConcurrentHashMap<>();
   ...
   public static TitanEyeEntity getClientInstance(int parentEntityId) {
   ...
   private void positionFromStaticOffset(TitanEntity parent, double scale) {
      ...
      double scaledForwardOffset = 2.0 * scale;
      double offsetX = -Math.sin(yawRad) * scaledForwardOffset;
      double offsetZ = Math.cos(yawRad) * scaledForwardOffset;
      this.setPosition(parent.getX() + offsetX, parent.getY() + 13.5 * scale, parent.getZ() + offsetZ);
   }

   public TitanEyeEntity(EntityType<? extends MobEntity> entityType, World level) {
   ...
   public boolean damage(DamageSource source, float amount) {
      if (this.getWorld().isClient()) {
         ...
         TitanEntity stunCheckParent = this.getParentTitan();
         if (stunCheckParent != null && stunCheckParent.isEating() && !ModConfig.get().canStunWhileGrabbed) {
            ...
            if (!(attacker instanceof AttackTitanEntity)
               ...
               TitanEntity ownerCheckParent = this.getParentTitan();
               ...
               if (source.isIn(DamageTypeTags.IS_EXPLOSION)) {
                  ...
                  if (attacker instanceof LivingEntity livingAttacker) {
                     ...
                     if (parent != null && !parent.isDead() && !parent.isEyeHurt()) {
                        ...
                        parent.triggerEyeHurt();
                        if (livingAttacker instanceof PlayerEntity player) {
                           player.sendMessage(Text.literal("Critical hit! Titan's eye damaged - stunned for 5 seconds!"), true);
                        }
