...
public class BeastTitanEyeEntity extends MobEntity {
   private static final TrackedData<Optional<UUID>> DATA_PARENT_UUID = DataTracker.registerData(
      BeastTitanEyeEntity.class, TrackedDataHandlerRegistry.OPTIONAL_UUID
   );
   private BeastTitanEntity cachedParent;
   private static final double EYE_OFFSET_FORWARD = 2.0;
   private static final double EYE_HEIGHT_OFFSET = 15.5;
   private static final Map<Integer, BeastTitanEyeEntity> clientInstances = new ConcurrentHashMap<>();
   ...
   public static BeastTitanEyeEntity getClientInstance(int parentEntityId) {
   ...
   private void positionFromStaticOffset(BeastTitanEntity parent) {
      ...
      this.setPosition(parent.getX() + offsetX, parent.getY() + 15.5, parent.getZ() + offsetZ);
   }

   public BeastTitanEyeEntity(EntityType<? extends MobEntity> entityType, World level) {
      ...
      if (this.getWorld().isClient()) {
         ...
         if (attacker instanceof LivingEntity livingAttacker) {
            ...
            if (parent != null && !parent.isBlinded()) {
               ...
               parent.triggerBlindness();
               if (livingAttacker instanceof PlayerEntity player) {
                  player.sendMessage(Text.literal("Critical hit! Beast Titan blinded for 6 seconds!"), true);
               }
