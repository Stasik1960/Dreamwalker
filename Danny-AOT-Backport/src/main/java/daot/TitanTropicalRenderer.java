...
public class TitanTropicalRenderer extends GeoEntityRenderer<TitanTropicalEntity> {
   ...
   public TitanTropicalRenderer(Context context) {
      super(context, new TitanTropicalModel());
   }

   public void render(
      TitanTropicalEntity entity, float entityYaw, float partialTick, MatrixStack poseStack, VertexConsumerProvider bufferSource, int packedLight
   ) {
      super.render(entity, entityYaw, partialTick, poseStack, bufferSource, packedLight);
      this.syncHitboxBones(entity, partialTick);
   }

   private void syncHitboxBones(TitanTropicalEntity entity, float partialTick) {
            ...
            if (!(distSq > 4096.0)) {
               ...
               if (napeBone == null && eyeBone == null) {
                  if (!loggedMissing.containsKey(entity.getId())) {
                     ...
                     System.out.println("[TitanTropicalHitboxSync] WARNING: nape_hitbox and eye_hitbox bones not found in model!");
                  }
               } else {
   ...
   protected float getDeathMaxRotation(TitanTropicalEntity animatable) {
      return 0.0F;
   }

   public int getPackedOverlay(TitanTropicalEntity animatable, float u, float partialTick) {
