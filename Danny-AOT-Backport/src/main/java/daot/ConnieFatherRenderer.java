...
public class ConnieFatherRenderer extends GeoEntityRenderer<ConnieFatherEntity> {
   private static final Map<Integer, Long> lastSendTick = new HashMap<>();

   public ConnieFatherRenderer(Context context) {
      super(context, new ConnieFatherModel());
   }

   public void render(
      ConnieFatherEntity entity, float entityYaw, float partialTick, MatrixStack poseStack, VertexConsumerProvider bufferSource, int packedLight
   ) {
      super.render(entity, entityYaw, partialTick, poseStack, bufferSource, packedLight);
      this.syncHitboxBones(entity, partialTick);
   }

   private void syncHitboxBones(ConnieFatherEntity entity, float partialTick) {
         ...
         if (lastTick == null || lastTick != currentTick) {
            ...
            if (!(distSq > 4096.0)) {
               ...
               if (napeBone != null || eyeBone != null) {
                  ...
                  ConnieFatherNapeEntity napeEntity = ConnieFatherNapeEntity.getClientInstance(entity.getId());
                  if (napeEntity != null && napeBone != null) {
                     napeEntity.setRendererPosition(napeX, napeY, napeZ);
                  }

                  ConnieFatherEyeEntity eyeEntity = ConnieFatherEyeEntity.getClientInstance(entity.getId());
   ...
   protected float getDeathMaxRotation(ConnieFatherEntity animatable) {
      return 0.0F;
   }

   public int getPackedOverlay(ConnieFatherEntity animatable, float u, float partialTick) {
