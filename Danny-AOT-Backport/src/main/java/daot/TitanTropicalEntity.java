...
public class TitanTropicalEntity extends FritzTitanEntity {
   private static final TrackedData<Integer> DATA_ABNORMAL_RUN = DataTracker.registerData(TitanTropicalEntity.class, TrackedDataHandlerRegistry.INTEGER);
   ...
   public TitanTropicalEntity(EntityType<? extends HostileEntity> entityType, World level) {
      super(entityType, level);
   }

   public static boolean checkTitanTropicalSpawnRules(
      EntityType<TitanTropicalEntity> entityType, ServerWorldAccess level, SpawnReason spawnType, BlockPos pos, Random random
   ) {
   ...
   protected FritzTitanNapeEntity createNapeHitbox() {
      return new TitanTropicalNapeEntity(DannysAot.TITAN_TROPICAL_NAPE, this.getWorld());
   }

   @Override
   protected FritzTitanEyeEntity createEyeHitbox() {
      return new TitanTropicalEyeEntity(DannysAot.TITAN_TROPICAL_EYE, this.getWorld());
   }
}
