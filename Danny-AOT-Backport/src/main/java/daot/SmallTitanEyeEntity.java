...
public class SmallTitanEyeEntity extends MobEntity implements PureTitanHitbox {
   private static final TrackedData<Optional<UUID>> DATA_PARENT_UUID = DataTracker.registerData(
      SmallTitanEyeEntity.class, TrackedDataHandlerRegistry.OPTIONAL_UUID
   );
   private SmallTitanEntity cachedParent;
   private static final double EYE_OFFSET_FORWARD = 0.65;
   private static final double EYE_HEIGHT_OFFSET = 3.2;
   private static final Map<Integer, SmallTitanEyeEntity> clientInstances = new ConcurrentHashMap<>();
   ...
   public static SmallTitanEyeEntity getClientInstance(int parentEntityId) {
   ...
   private void positionFromStaticOffset(SmallTitanEntity parent, double scale) {
      float yawRad = (float)Math.toRadians(parent.bodyYaw);
      double scaledForwardOffset = 0.65 * scale;
      double offsetX = -Math.sin(yawRad) * scaledForwardOffset;
      double offsetZ = Math.cos(yawRad) * scaledForwardOffset;
      this.setPosition(parent.getX() + offsetX, parent.getY() + 3.2 * scale, parent.getZ() + offsetZ);
   }

   public SmallTitanEyeEntity(EntityType<? extends MobEntity> entityType, World level) {
   ...
   public void setParentTitan(SmallTitanEntity titan) {
   ...
   public SmallTitanEntity getParentTitan() {
      ...
      if (parentUUID.isEmpty()) {
         ...
         if (this.getWorld() instanceof ServerWorld serverLevel && serverLevel.getEntity(uuid) instanceof SmallTitanEntity titan && titan.isAlive()) {
            this.cachedParent = titan;
            return titan;
         } else {
            if (this.getWorld().isClient()) {
               for (Entity entity : this.getWorld().getOtherEntities(this, this.getBoundingBox().expand(20.0))) {
                  if (entity instanceof SmallTitanEntity titan && titan.getUuid().equals(uuid)) {
                     this.cachedParent = titan;
                     return titan;
                  }
               }
   ...
   public void tick() {
      super.tick();
      SmallTitanEntity parent = this.getParentTitan();
      if (parent == null) {
   ...
   public boolean damage(DamageSource source, float amount) {
      if (this.getWorld().isClient()) {
         ...
         SmallTitanEntity stunCheckParent = this.getParentTitan();
         if (stunCheckParent != null && stunCheckParent.isEating() && !ModConfig.get().canStunWhileGrabbed) {
            ...
            if (!(attacker instanceof AttackTitanEntity)
               ...
               SmallTitanEntity ownerCheckParent = this.getParentTitan();
               ...
               if (source.isIn(DamageTypeTags.IS_EXPLOSION)) {
                  ...
                  if (attacker instanceof LivingEntity livingAttacker) {
                     ...
                     if (parent != null && !parent.isDead() && !parent.isEyeHurt()) {
                        ...
                        parent.triggerEyeHurt();
                        if (livingAttacker instanceof PlayerEntity player) {
                           player.sendMessage(Text.literal("Critical hit! Small Titan's eye damaged - stunned for 5 seconds!"), true);
                        }
   ...
   public boolean collidesWith(Entity other) {
      return other instanceof SmallTitanEntity titan ? titan != this.getParentTitan() : super.collidesWith(other);
   }
