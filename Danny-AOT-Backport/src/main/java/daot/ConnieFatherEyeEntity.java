...
public class ConnieFatherEyeEntity extends MobEntity implements PureTitanHitbox {
   private static final TrackedData<Optional<UUID>> DATA_PARENT_UUID = DataTracker.registerData(
      ConnieFatherEyeEntity.class, TrackedDataHandlerRegistry.OPTIONAL_UUID
   );
   private ConnieFatherEntity cachedParent;
   private static final double EYE_OFFSET_FORWARD = 0.5;
   private static final double EYE_HEIGHT_OFFSET = 2.5;
   private static final Map<Integer, ConnieFatherEyeEntity> clientInstances = new ConcurrentHashMap<>();
   ...
   public static ConnieFatherEyeEntity getClientInstance(int parentEntityId) {
   ...
   private void positionFromStaticOffset(ConnieFatherEntity parent, double scale) {
      float yawRad = (float)Math.toRadians(parent.bodyYaw);
      double scaledForwardOffset = 0.5 * scale;
      double offsetX = -Math.sin(yawRad) * scaledForwardOffset;
      double offsetZ = Math.cos(yawRad) * scaledForwardOffset;
      this.setPosition(parent.getX() + offsetX, parent.getY() + 2.5 * scale, parent.getZ() + offsetZ);
   }

   public ConnieFatherEyeEntity(EntityType<? extends MobEntity> entityType, World level) {
   ...
   public void setParentTitan(ConnieFatherEntity titan) {
   ...
   public ConnieFatherEntity getParentTitan() {
      ...
      if (parentUUID.isEmpty()) {
         ...
         if (this.getWorld() instanceof ServerWorld serverLevel && serverLevel.getEntity(uuid) instanceof ConnieFatherEntity titan && titan.isAlive()) {
            this.cachedParent = titan;
            return titan;
         } else {
            if (this.getWorld().isClient()) {
               for (Entity entity : this.getWorld().getOtherEntities(this, this.getBoundingBox().expand(20.0))) {
                  if (entity instanceof ConnieFatherEntity titan && titan.getUuid().equals(uuid)) {
                     this.cachedParent = titan;
                     return titan;
                  }
               }
   ...
   public void tick() {
      super.tick();
      ConnieFatherEntity parent = this.getParentTitan();
      if (parent == null) {
   ...
   public boolean damage(DamageSource source, float amount) {
      if (this.getWorld().isClient()) {
         ...
         ConnieFatherEntity stunCheckParent = this.getParentTitan();
         if (stunCheckParent != null && stunCheckParent.isEating() && !ModConfig.get().canStunWhileGrabbed) {
            ...
            if (!(attacker instanceof AttackTitanEntity)
               ...
               ConnieFatherEntity ownerCheckParent = this.getParentTitan();
               ...
               if (source.isIn(DamageTypeTags.IS_EXPLOSION)) {
                  ...
                  if (attacker instanceof LivingEntity livingAttacker) {
                     ...
                     if (parent != null && !parent.isDead() && !parent.isEyeHurt()) {
                        ...
                        parent.triggerEyeHurt();
                        if (livingAttacker instanceof PlayerEntity player) {
                           player.sendMessage(Text.literal("Critical hit! Connie's Father Titan's eye damaged - stunned for 5 seconds!"), true);
                        }
   ...
   public boolean collidesWith(Entity other) {
      return other instanceof ConnieFatherEntity titan ? titan != this.getParentTitan() : super.collidesWith(other);
   }
