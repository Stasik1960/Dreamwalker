...
public class SmallTitan2EyeEntity extends MobEntity implements PureTitanHitbox {
   private static final TrackedData<Optional<UUID>> DATA_PARENT_UUID = DataTracker.registerData(
      SmallTitan2EyeEntity.class, TrackedDataHandlerRegistry.OPTIONAL_UUID
   );
   ...
   private static final Map<Integer, SmallTitan2EyeEntity> clientInstances = new ConcurrentHashMap<>();
   ...
   public static SmallTitan2EyeEntity getClientInstance(int parentEntityId) {
   ...
   public SmallTitan2EyeEntity(EntityType<? extends MobEntity> entityType, World level) {
   ...
   public boolean damage(DamageSource source, float amount) {
      if (this.getWorld().isClient()) {
         ...
         SmallTitan2Entity stunCheckParent = this.getParentTitan();
         if (stunCheckParent != null && stunCheckParent.isEating() && !ModConfig.get().canStunWhileGrabbed) {
            ...
            if (!(attacker instanceof AttackTitanEntity)
               ...
               SmallTitan2Entity ownerCheckParent = this.getParentTitan();
               ...
               if (source.isIn(DamageTypeTags.IS_EXPLOSION)) {
                  ...
                  if (attacker instanceof LivingEntity livingAttacker) {
                     ...
                     if (parent != null && !parent.isDead() && !parent.isEyeHurt()) {
                        ...
                        parent.triggerEyeHurt();
                        if (livingAttacker instanceof PlayerEntity player) {
                           player.sendMessage(Text.literal("Critical hit! Small Titan 2's eye damaged - stunned for 5 seconds!"), true);
                        }
