...
public class ODMAPGItem extends daot.compat.BackportArmorItem implements GeoItem {
   ...
   public ODMAPGItem(ArmorMaterial material, Type type, Settings properties) {
   ...
   public static void setGas(ItemStack stack, int amount) {
      int clampedAmount = Math.max(0, Math.min(1000, amount));
   ...
   public void appendTooltip(ItemStack stack, World context, List<Text> tooltip, TooltipContext type) {
      ...
      Formatting gasColor = gas > 500 ? Formatting.AQUA : (gas > 200 ? Formatting.YELLOW : Formatting.RED);
      tooltip.add(Text.literal("Gas: " + gas + "/1000").formatted(gasColor));
      tooltip.add(Text.literal("Equip APG Guns in main & offhand to use").formatted(Formatting.RED));
