...
public class YellowTitanNapeEntity extends MobEntity {
   private static final TrackedData<Optional<UUID>> DATA_PARENT_UUID = DataTracker.registerData(
      YellowTitanNapeEntity.class, TrackedDataHandlerRegistry.OPTIONAL_UUID
   );
   ...
   private static final Map<Integer, YellowTitanNapeEntity> clientInstances = new ConcurrentHashMap<>();
   ...
   public static YellowTitanNapeEntity getClientInstance(int parentEntityId) {
   ...
   public YellowTitanNapeEntity(EntityType<? extends MobEntity> entityType, World level) {
   ...
   public boolean damage(DamageSource source, float amount) {
      if (this.getWorld().isClient()) {
         ...
         if (!(attacker instanceof AttackTitanEntity)
            ...
            YellowTitanEntity unfairParent = this.getParentTitan();
               ...
               if (parent != null && parent.hasPassenger(attacker)) {
                  ...
                  if ((isUsingBlade || isHomelander) && isMeleeRange && parent != null) {
                     ...
                     if (livingAttacker instanceof PlayerEntity player) {
                        player.sendMessage(Text.literal("Critical hit! Yellow Titan's nape destroyed!"), true);
                     }
