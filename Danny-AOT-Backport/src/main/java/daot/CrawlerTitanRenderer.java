...
public class CrawlerTitanRenderer extends GeoEntityRenderer<CrawlerTitanEntity> {
   private static final Map<Integer, Long> lastSendTick = new HashMap<>();

   public CrawlerTitanRenderer(Context context) {
      super(context, new CrawlerTitanModel());
   }

   public void render(
      CrawlerTitanEntity entity, float entityYaw, float partialTick, MatrixStack poseStack, VertexConsumerProvider bufferSource, int packedLight
   ) {
      super.render(entity, entityYaw, partialTick, poseStack, bufferSource, packedLight);
      this.syncHitboxBones(entity, partialTick);
   }

   private void syncHitboxBones(CrawlerTitanEntity entity, float partialTick) {
         ...
         if (lastTick == null || lastTick != currentTick) {
            ...
            if (!(distSq > 4096.0)) {
               ...
               if (napeBone != null || eyeBone != null) {
                  ...
                  CrawlerTitanNapeEntity napeEntity = CrawlerTitanNapeEntity.getClientInstance(entity.getId());
                  if (napeEntity != null && napeBone != null) {
                     napeEntity.setRendererPosition(napeX, napeY, napeZ);
                  }

                  CrawlerTitanEyeEntity eyeEntity = CrawlerTitanEyeEntity.getClientInstance(entity.getId());
   ...
   protected float getDeathMaxRotation(CrawlerTitanEntity animatable) {
      return 0.0F;
   }

   public int getPackedOverlay(CrawlerTitanEntity animatable, float u, float partialTick) {
