...
public class TitanDummyEyeEntity extends MobEntity {
   private static final TrackedData<Optional<UUID>> DATA_PARENT_UUID = DataTracker.registerData(
      TitanDummyEyeEntity.class, TrackedDataHandlerRegistry.OPTIONAL_UUID
   );
   private TitanDummyEntity cachedParent;
   private static final Map<Integer, TitanDummyEyeEntity> clientInstances = new ConcurrentHashMap<>();
   ...
   private static final double EYE_HEIGHT_OFFSET = 7.5;
   private static final double EYE_OFFSET_FORWARD = 2.0;

   public TitanDummyEyeEntity(EntityType<? extends MobEntity> entityType, World level) {
   ...
   public static TitanDummyEyeEntity getClientInstance(int parentEntityId) {
      ...
      if (this.getWorld().isClient()) {
         ...
         if (parentx != null && !parentx.isRemoved()) {
            if (this.lastSyncTick >= 0L && this.getWorld().getTime() - this.lastSyncTick < 10L) {
               ...
               this.setPosition(parentx.getX() + offsetX, parentx.getY() + 7.5, parentx.getZ() + offsetZ);
            }
         } else {
            this.discard();
         }
      ...
      if (this.getWorld().isClient()) {
         ...
         if (!isUsingBlade) {
            if (livingAttacker instanceof PlayerEntity player) {
               player.sendMessage(Text.literal("Use a blade to hit the eyes!"), true);
            }
            ...
            TitanDummyEntity parent = this.getParentDummy();
            if (parent != null) {
               parent.triggerEyeStun();
               if (livingAttacker instanceof PlayerEntity player) {
                  player.sendMessage(Text.literal("Eye hit! Dummy stunned for 5 seconds!"), true);
               }
