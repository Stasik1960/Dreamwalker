...
public class TitanNapeEntity extends MobEntity {
   private static final TrackedData<Optional<UUID>> DATA_PARENT_UUID = DataTracker.registerData(TitanNapeEntity.class, TrackedDataHandlerRegistry.OPTIONAL_UUID);
   private TitanEntity cachedParent;
   private static final double NAPE_OFFSET_BACK = 1.5;
   private static final double NAPE_HEIGHT_OFFSET = 12.0;
   private static final Map<Integer, TitanNapeEntity> clientInstances = new ConcurrentHashMap<>();
   ...
   public static TitanNapeEntity getClientInstance(int parentEntityId) {
   ...
   private void positionFromStaticOffset(TitanEntity parent, double scale) {
      float yawRad = (float)Math.toRadians(parent.bodyYaw);
      double scaledBackOffset = 1.5 * scale;
      double offsetX = Math.sin(yawRad) * scaledBackOffset;
      double offsetZ = -Math.cos(yawRad) * scaledBackOffset;
      this.setPosition(parent.getX() + offsetX, parent.getY() + 12.0 * scale, parent.getZ() + offsetZ);
   }

   public TitanNapeEntity(EntityType<? extends MobEntity> entityType, World level) {
   ...
   public TitanEntity getParentTitan() {
      ...
      if (parentUUID.isEmpty()) {
         ...
         if (this.getWorld() instanceof ServerWorld serverLevel && serverLevel.getEntity(uuid) instanceof TitanEntity titan && titan.isAlive()) {
            this.cachedParent = titan;
            return titan;
         } else {
            if (this.getWorld().isClient()) {
               for (Entity entity : this.getWorld().getOtherEntities(this, this.getBoundingBox().expand(50.0))) {
                  if (entity instanceof TitanEntity titan && titan.getUuid().equals(uuid)) {
                     this.cachedParent = titan;
                     return titan;
                  }
               }
   ...
   @Override
   public void lookAt(EntityAnchor anchorPoint, Vec3d target) {
   }
   ...
   public void tick() {
      super.tick();
      TitanEntity parent = this.getParentTitan();
      if (parent == null) {
   ...
   public boolean damage(DamageSource source, float amount) {
      if (this.getWorld().isClient()) {
         ...
         if (!(attacker instanceof AttackTitanEntity)
            ...
            TitanEntity unfairParent = this.getParentTitan();
            if (unfairParent != null
               ...
               if (parent != null && parent.hasPassenger(attacker)) {
                  ...
                  if ((isUsingBlade || isHomelander) && parent != null) {
                     ...
                     if (livingAttacker instanceof PlayerEntity player) {
                        player.sendMessage(Text.literal("Critical hit! Titan's nape destroyed!"), true);
                     }
   ...
   public boolean collidesWith(Entity other) {
      return other instanceof TitanEntity titan ? titan != this.getParentTitan() : super.collidesWith(other);
   }
