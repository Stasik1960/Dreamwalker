...
public class FritzTitanNapeEntity extends MobEntity {
   private static final TrackedData<Optional<UUID>> DATA_PARENT_UUID = DataTracker.registerData(
      FritzTitanNapeEntity.class, TrackedDataHandlerRegistry.OPTIONAL_UUID
   );
   private FritzTitanEntity cachedParent;
   private static final double NAPE_OFFSET_BACK = 1.2;
   private static final double NAPE_HEIGHT_OFFSET = 11.5;
   ...
   private static final Map<Integer, FritzTitanNapeEntity> clientInstances = new ConcurrentHashMap<>();
   ...
   public static FritzTitanNapeEntity getClientInstance(int parentEntityId) {
   ...
   public FritzTitanNapeEntity(EntityType<? extends MobEntity> entityType, World level) {
   ...
   public static net.minecraft.entity.attribute.DefaultAttributeContainer.Builder createAttributes() {
      return MobEntity.createMobAttributes()
         ...
         .add(daot.compat.attributes.DaotEntityAttributes.SCALE, 0.8);
   }

   public void setParentTitan(FritzTitanEntity titan) {
      this.dataTracker.set(DATA_PARENT_UUID, Optional.of(titan.getUuid()));
      this.cachedParent = titan;
   }

   public FritzTitanEntity getParentTitan() {
      ...
      if (parentUUID.isEmpty()) {
         ...
         if (this.getWorld() instanceof ServerWorld serverLevel && serverLevel.getEntity(uuid) instanceof FritzTitanEntity titan && titan.isAlive()) {
            this.cachedParent = titan;
            return titan;
         } else {
            if (this.getWorld().isClient()) {
               for (Entity entity : this.getWorld().getOtherEntities(this, this.getBoundingBox().expand(20.0))) {
                  if (entity instanceof FritzTitanEntity titan && titan.getUuid().equals(uuid)) {
                     this.cachedParent = titan;
                     return titan;
                  }
               }
   ...
   public void tick() {
      super.tick();
      FritzTitanEntity parent = this.getParentTitan();
      if (parent == null) {
         ...
         if (!this.getWorld().isClient() && this.getAttributeInstance(daot.compat.attributes.DaotEntityAttributes.SCALE) != null) {
            this.getAttributeInstance(daot.compat.attributes.DaotEntityAttributes.SCALE).setBaseValue(scale * 0.8);
         }
   ...
   private void positionFromStaticOffset(FritzTitanEntity parent, double scale) {
      float yawRad = (float)Math.toRadians(parent.bodyYaw);
      double scaledBackOffset = this.getNapeBackOffset() * scale;
      double offsetX = Math.sin(yawRad) * scaledBackOffset;
      double offsetZ = -Math.cos(yawRad) * scaledBackOffset;
      this.setPosition(parent.getX() + offsetX, parent.getY() + this.getNapeHeightOffset() * scale, parent.getZ() + offsetZ);
   }
   ...
   public boolean damage(DamageSource source, float amount) {
      if (this.getWorld().isClient()) {
         ...
         if (!(attacker instanceof AttackTitanEntity)
            ...
            FritzTitanEntity unfairParent = this.getParentTitan();
               ...
               if (parent != null && parent.hasPassenger(attacker)) {
                  ...
                  if ((isUsingBlade || isHomelander) && isMeleeRange && parent != null) {
                     ...
                     if (livingAttacker instanceof PlayerEntity player) {
                        String titanName = parent.getName().getString();
                        player.sendMessage(Text.literal("Critical hit! " + titanName + "'s nape destroyed!"), true);
                     }
   ...
   public boolean collidesWith(Entity other) {
      return other instanceof FritzTitanEntity titan ? titan != this.getParentTitan() : super.collidesWith(other);
   }
