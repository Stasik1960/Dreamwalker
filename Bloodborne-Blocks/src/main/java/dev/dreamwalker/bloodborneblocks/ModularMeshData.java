...
final class ModularMeshData {
 ...
 static Map<String,Mesh> loadAndValidate(){
  ...
  try(InputStream stream=ModularMeshData.class.getResourceAsStream("/bloodborne_blocks/v2/meshes.json.gz")){
   ...
   try(JsonReader reader=new JsonReader(new InputStreamReader(new GZIPInputStream(stream),StandardCharsets.UTF_8))){
    ...
    while(reader.hasNext()){
     String id=reader.nextName();require(id.startsWith("m_"),"Invalid modular mesh "+id);
     Mesh mesh=readMesh(reader,id,textures);if(meshes.putIfAbsent(id,mesh)!=null)throw new IllegalStateException("Duplicate modular mesh "+id);
    }
 ...
 private static Mesh readMesh(JsonReader reader,String id,Map<String,String> textures)throws IOException{
  require(reader.peek()==JsonToken.BEGIN_OBJECT,"Invalid modular mesh "+id);reader.beginObject();List<Polygon> polygons=null;
  while(reader.hasNext()){
   String name=reader.nextName();
   if(name.equals("polygons")){require(polygons==null,"Duplicate polygons field "+id);polygons=readPolygons(reader,id,textures);}
   else reader.skipValue();
  }
  reader.endObject();require(polygons!=null,"Missing polygons "+id);return new Mesh(polygons);
 }

 private static List<Polygon> readPolygons(JsonReader reader,String id,Map<String,String> textures)throws IOException{
  ...
  while(reader.hasNext())result.add(readPolygon(reader,id,index++,textures));reader.endArray();return result;
 }

 private static Polygon readPolygon(JsonReader reader,String id,int polygonIndex,Map<String,String> textures)throws IOException{
  ...
  while(reader.hasNext()){
   ...
   else if(name.equals("vertices")){require(vertices==null,"Duplicate vertices "+label(id,polygonIndex));vertices=readVertices(reader,id,polygonIndex);}
   else reader.skipValue();
  }
 ...
 private static float[] readVertices(JsonReader reader,String id,int polygonIndex)throws IOException{
  ...
  while(reader.hasNext()){
   ...
   for(int component=0;component<5;component++){
    ...
    require(value>=0&&value<=(component<3?1:16),(component<3?"Out-of-cell vertex ":"Out-of-range UV ")+label(id,polygonIndex)+"["+vertexIndex+"]");packed.add((float)value);
   }
