...
public final class LogicalRuntimeGameTests implements FabricGameTest {
 ...
 private static BlockPos find(TestContext context,Block block){ServerWorld world=context.getWorld();for(int x=-4;x<12;x++)for(int y=1;y<9;y++)for(int z=-4;z<12;z++){BlockPos pos=context.getAbsolutePos(new BlockPos(x,y,z));if(world.getBlockState(pos).isOf(block))return pos;}return null;}
 ...
 private static void clear(TestContext context,ArchitectureBlock block){ServerWorld world=context.getWorld();for(int x=-4;x<12;x++)for(int y=1;y<9;y++)for(int z=-4;z<12;z++){BlockPos pos=context.getAbsolutePos(new BlockPos(x,y,z));BlockState state=world.getBlockState(pos);if(state.isOf(block))world.breakBlock(pos,false);}}
