...
public final class ProductionRestorationGameTests implements FabricGameTest {
 ...
 public void lanternsBenchesLaddersAndConnectionsBehave(TestContext context){
  ...
  try{
   ...
   for(String id:List.of("o_bench","o_bench_rotate")){ArchitectureBlock bench=required(id);world.setBlockState(root,bench.getDefaultState(),Block.NOTIFY_ALL);GeometryRuntime.rebuild(world,root,world.getBlockState(root));context.useBlock(ROOT,player);context.assertTrue(player.hasVehicle(),"bench mounts player "+id);context.assertTrue(Math.abs(player.getVehicle().getY()-root.getY()-1)<.001,"seat plank height, not clipped backrest cap "+id);player.stopRiding();clear(context,bench);}
   for(String id:List.of("o_ladder_01","o_ladder_02","o_ladder_03")){ArchitectureBlock ladder=required(id);world.setBlockState(root,ladder.getDefaultState(),Block.NOTIFY_ALL);GeometryRuntime.rebuild(world,root,world.getBlockState(root));context.assertTrue(FunctionalFurniture.isClimbable(world,root),"ladder root climbs "+id);BlockPos helper=firstHelper(root,world.getBlockState(root));if(helper!=null)context.assertTrue(FunctionalFurniture.isClimbable(world,helper),"ladder helper climbs "+id);clear(context,ladder);}
