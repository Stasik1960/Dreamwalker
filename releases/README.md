...
Устранён блокирующий запрос чанка из обработчика загрузки, найденный в двух снимках зависшего сервера. Для Ether используйте последний локальный архив **2.0.2-positions**: он включает исправление света, удаление барьеров и восстановление положения объектов. Версия JAR остаётся **2.0.1**; повторная миграция карты не требуется. [Исправление загрузки](../Bloodborne-Blocks/docs/SPAWN-LOAD-FIX-2.0.1-RU.md) · [Исправление карты](../Bloodborne-Blocks/docs/MAP-POSITIONS-2.0.2-RU.md).

Строительная палитра разделена на отдельные клетки с простыми коллизиями и поворотами. Большие двери и мебель остаются целыми. Мод устанавливается на клиент и сервер Fabric 1.20.1 / Java 17. Переработанная карта выдаётся локально отдельно; заменить JAR недостаточно для преобразования прежней карты. [Описание пересборки и фактических проверок](../Bloodborne-Blocks/docs/REBUILD-2.0-RU.md).
